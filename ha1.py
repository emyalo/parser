...
def main_parser():
    headers = {"User-Agent": "Mozilla/5.0 (Macintosh; Intel Mac OS X 10.0; rv:70.0) Gecko/20100101 Firefox/70.0"}
...
def open_data_json():
    with open ('1_3_special_offers','r') as f:
        results = json.load(f)
        return results

#MAIN
results = main_parser()
#save_data_json('1_3_special_offers',results) #ПРИМЕЧАНИЕ: сайт часто не открываеся, ссылка 'https://5ka.ru/api/v2/special_offers/' возвращает пусто. Поэтому для отладки сохраним результат в файл
